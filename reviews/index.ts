...
const resolvers = {
  Book: {
    reviews: (book: { id: string }) => {
      return reviews.filter(review => review.bookId === book.id);
    }
  },
  Query: {
    reviews: (_: any, { bookId, reviewId }: { bookId: string; reviewId?: string }) => {
  ...
  Mutation: {
    addReview: (_: any, { bookId, rating, comment }: { bookId: string; rating: number; comment?: string }) => {
      const newReview = {
